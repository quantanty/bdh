...
class Attention(torch.nn.Module):
    ...
    def forward(self, Q, K, V, decay, cache_method=None, layer_state=None, t_offset=0):
        assert K is Q
        ...
        # Current attention
        a = ((Q @ K.mT).tril(diagonal=-1) * decay) @ V
        if layer_state is not None:
            a = a * (1-self.config.u)**T +  Q @ layer_state
            if cache_method == 'state':
                layer_state += K.mT @ V.expand(-1, H, -1, -1)
...
class BDH(nn.Module):
    def __init__(self, config: BDHConfig):
        ...
        self.powers = torch.nn.Buffer(
            (1 - self.config.u) ** torch.arange(12, dtype=torch.float32)
        )
    ...
    def forward(self, idx, targets=None, cache_method=None, state=None, t_offset=0):
        ...
        if self.powers.size(0) < T:
            self.powers = torch.cat((self.powers, (1 - self.config.u) ** torch.arange(self.powers.size(0), T, dtype=torch.float32, device=self.powers.device)))
        powers = self.powers[:T].flip(0)
        power_idx = torch.arange(T, device=powers.device)
        i = power_idx.view(T, 1)
        j = power_idx.view(1, T)

        mask = j <= i
        decay = torch.zeros(T, T, device=powers.device)
        decay[mask] = powers[(T - 1 - (i - j))[mask]]
        del power_idx, i, j, mask
